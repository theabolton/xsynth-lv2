 ...
 *
 * - This version works with GTK+ 1.2 or later.
...
static void     gtk_knob_finalize      (GtkObject *object);
...
#else /* GTK_CHECK_VERSION(2, 0, 0) */
    object_class->finalize = gtk_knob_finalize;
#endif /* GTK_CHECK_VERSION(2, 0, 0) */
...
#else /* GTK_CHECK_VERSION(2, 0, 0) */
gtk_knob_finalize (GtkObject *object)
#endif /* GTK_CHECK_VERSION(2, 0, 0) */
{
...
gtk_knob_destroy (GtkObject *object)
{
    ...
    if (knob->adjustment) {
        gtk_signal_disconnect_by_func (GTK_OBJECT(knob->adjustment),
...
#if !GTK_CHECK_VERSION(2, 0, 0)
        /* Drawing the pixmap before the arc (and then again after) helps minimize
         * flashing. There's gotta be a better way, though (especially since it's
         * only needed for GTK+ 1.2.... */
